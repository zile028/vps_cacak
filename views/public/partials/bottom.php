<!-------- FOOTER -------->

<footer>
    <article class="container">
        <div>
            <a class="logo" href="/">
                <img src="<?php echo uploadPath("logo-white.png"); ?>" alt="vps zemun logo">
                <span>Висока пословна школа<br>струковних студија Чачак</span>
            </a>
            <p>Одлучити о томе где наставити школовање после средње школе или где се дошколовати уз
                рад, једна је од
                највећих животних одлука. Она ће пресудно утицати на ваш даљи живот. ВПШ Чачак вам
                помаже да стекнете
                неопходна знања и квалификације које су потребне за успех у каријери.</p>
        </div>

        <div>
            <h3>Контакт</h3>
            <ul class="contact-footer p-t-28">
                <li>
                    <i class="fas fa-home" aria-hidden="true"></i>
                    <span><?php echo ADRESA; ?></span>
                </li>
                <li>
                    <i class="fas fa-phone" aria-hidden="true"></i>
                    <span><?php echo TELEFON; ?></span>
                </li>
                <li class="ignore">
                    <i class="fas fa-envelope" aria-hidden="true"></i>
                    <span><?php echo EMAIL; ?></span>
                </li>
                <li>
                    <i class="fas fa-clock" aria-hidden="true"></i>
                    <span>Радно време: <?php echo RADNO_VREME; ?></span>
                </li>
            </ul>
        </div>
    </article>
    <article class="ignore">
        <p class="s-txt10"> &COPY; 2024. Dejan Živković - zile028@gmail.com</p>
    </article>
</footer>

<script src="assets/public/js/main.js"></script>
</body>

</html>
