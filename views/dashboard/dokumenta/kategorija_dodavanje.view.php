...
<div class="card">
    <div class="card-body">
        <form action="/document/category" class="row" method="post">
            <div class="row">
                <div class="col-md-4">
    ...
    <div class="card-body">
        <table class="table">
            ...
            <?php foreach ($kategorije as $lang => $kategorija) : ?>
                <tr>
                    <th class="text-uppercase fw-bold bg-body" colspan="3">
                        <?php echo $lang; ?>
                    </th>
                </tr>
                <?php foreach ($kategorija as $item): ?>
                    <tr>
                        <td>
                            <?php updateSingle("/document/category/" . $item->id, "category", $item->category); ?>
                        </td>
                        <td>
                            <?php updatePriority("/document/category/" . $item->id, $item->prioritet); ?>
                        </td>
                        <td>
                            <?php updateSingle("/document/category/" . $item->id, "slug", $item->slug); ?>
                        </td>
                        <td>
                            <?php updateSingle("/document/category/" . $item->id, "lang", $item->lang); ?>
                        </td>
                        <td style="width: 15%">
                            <?php if ($item->count > 0): ?>
                                <p>Већ постоје документа!</p>
                            <?php else: ?>
                                <?php deleteForm("/document/category/" . $item->id); ?>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
